...
(defun run-all-tests ()
  ...
  ;;; XXX Are the yin-yang ops over the clan correct?
   ...
   (is? (with-sets
            ((outlaws (@clan
                       (@r (@c 1 'id) (@c "Robin Hood" 'name) (@c 'larceny 'crime)
                           (@c "100 guineas" 'reward) (@c "Locksley" 'born-in)
                           (@c "Sherwood Forest" 'last-seen))
                       (@r (@c 2 'id) (@c "Billy The Kid" 'name) (@c 'murder 'crime)
                           (@c "$500" 'reward) (@c "New York" 'born-in) (@c "On the 1am" 'last-seen))
                       (@r (@c 3 'id) (@c "Al Capone" 'name) (@c "Tax Evasion" 'crime)
                           (@c "$500" 'reward) (@c "New York" 'born-in) (@c "Frank Nitti's speakeasy" 'last-seen))
                       (@r (@c 4 'id) (@c "Loki" 'name) (@c "Deception" 'crime)
                           (@c "Thor's Gratitude" 'reward) (@c "Asgard" 'born-in) (@c "On Earth Goddamit!" 'last-seen))
                       (@r (@c 5 'id) (@c "Chewbacca" 'name) (@c "Treason" 'crime)
                           (@c "Bounty" 'reward) (@c "Kashyyyk" 'born-in) (@c "Tatouine" 'last-seen))
                       )
                      )
             (filter (@clan (@r (@c 'name 'name) (@c 'crime 'crime) (@c 'last-seen 'last-seen))))
             )
          (@compose outlaws filter)
          )
        (@clan
         (@r (@c "Robin Hood" 'name) (@c 'larceny 'crime)
             (@c "Sherwood Forest" 'last-seen))
         (@r (@c "Billy The Kid" 'name) (@c 'murder 'crime)
             (@c "On the 1am" 'last-seen))
         (@r (@c "Al Capone" 'name) (@c "Tax Evasion" 'crime)
             (@c "Frank Nitti's speakeasy" 'last-seen))
         (@r (@c "Loki" 'name) (@c "Deception" 'crime)
             (@c "On Earth Goddamit!" 'last-seen))
         (@r (@c "Chewbacca" 'name) (@c "Treason" 'crime)
             (@c "Tatouine" 'last-seen))
         )
        )
   )
  )
